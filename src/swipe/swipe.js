...
class SimpleKeyboardSwipe {
  init(keyboard) {
    keyboard.registerModule("swipe", module => {
      ...
      module.init = () => {
        // Custom modification to collect swiping positions
        module.swipingPositions = []
      ...
      module.touchHandler = event => {
        ...
        module.swipingPositions.push([Math.round(event.timeStamp), Math.round(first.clientX), Math.round(first.clientY)])
        module.canvasHandler.canvas.dispatchEvent(simulatedEvent);
        //event.preventDefault();
      };

      module.clearSwipingPositions = () => {
        module.swipingPositions.length = 0
      }
